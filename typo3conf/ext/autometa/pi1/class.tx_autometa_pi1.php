...
class tx_autometa_pi1 extends tslib_pibase {
	...
	function main(&$content, $config, $extConf) {
		...
		$this->meta_keywords    ($this->content);
		$this->meta_description ($this->content);
		$this->meta_date        ($this->content);
		$this->meta_author      ($this->content);
		$this->meta_generator   ($this->content);
	 ...
	 * @return  void
	 */
	function meta_description(&$html) {
	...
	/**
	 * Is added via pure TypoScript but will be removed, if Doctype is HTML5
	 *
	 * @param   string  plain html of the document, passed by reference
	 * @return  void
	 */
	function meta_date(&$html) {
		if ($GLOBALS['TSFE']->config['config']['doctype'] == 'html_5' && $this->config['date.']['html5fix']) {
			$html = preg_replace (
				'/\t?<meta name=\"?date\"?.+?>\n?/is',
				'',
				$html
	...
	function meta_generator(&$html) {
		...
		} else if ($this->config['generator.']['tokens'] == 'Prod' || $this->config['generator.']['tokens'] == 'ProductOnly') {
			$html = preg_replace (
				'/(<meta name=\"generator\".+?)( \d\.\d)([^>]*?>)/is',
				'$1$3',
				$html
			);
		} else if ($this->config['generator.']['tokens'] == 'Major') {
			$html = preg_replace (
				'/(<meta name=\"generator\".+? \d)(\.\d)([^>]*?>)/is',
				'$1$3',
				$html
		...
		if ($strip_title === true) {
			if (empty($this->config['description.']['title.']['regex'])) {
				...
				if(@preg_match($this->config['description.']['title.']['regex'], $this->content, $title) !== false) {
					...
					if ($this->extConf['enableDevLog'] == 1) { t3lib_div::devLog('determined title', $this->extKey, 0, array('title' => $title[1], 'title_pattern' => $this->config['description.']['title.']['regex'])); }
				} else {
	...
	function get_keywords($str, $amount) {
		...
		foreach($arr as $key => $value) {
			if (
				strlen($value) <= 2 ||                   // keys shorter/equal 2 characters
